  ...
  <form name="selectPageForm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
    ...
    <?php $_from = $this->_var['pager']['search']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'item_0_83807900_1515501884');if (count($_from)):
    foreach ($_from AS $this->_var['key'] => $this->_var['item_0_83807900_1515501884']):
?> 
    <?php if ($this->_var['key'] == 'keywords'): ?>
    <input type="hidden" name="<?php echo $this->_var['key']; ?>" value="<?php echo urldecode($this->_var['item_0_83807900_1515501884']); ?>" />
    <?php else: ?>
    <input type="hidden" name="<?php echo $this->_var['key']; ?>" value="<?php echo $this->_var['item_0_83807900_1515501884']; ?>" />
    <?php endif; ?> 
    ...
    <div class="pagenav" id="pagenav">
      <ul>
        <li> 
          ...
          <?php $_from = $this->_var['pager']['page_number']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'item_0_83834400_1515501884');if (count($_from)):
    foreach ($_from AS $this->_var['key'] => $this->_var['item_0_83834400_1515501884']):
?> 
          <?php if ($this->_var['pager']['page'] == $this->_var['key']): ?> 
          <span class="currentStep"><?php echo $this->_var['key']; ?></span> 
          <?php else: ?> 
          <a href="<?php echo $this->_var['item_0_83834400_1515501884']; ?>" class="step"><?php echo $this->_var['key']; ?></a> 
          <?php endif; ?> 
          <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?> 
          <?php endif; ?> 
